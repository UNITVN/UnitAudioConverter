...
public class UAConverter {
    ...
    public func convert(source: URL, destination: URL, fileType:UAFileType) -> UAConvertSession {
            ...
            || fileType == .flac
        {
        ...
        } else if fileType == .aifc || fileType == .aiff {
            if self.checkAudioFileAccessibility(fileURL: source) {
                debugPrint("Warning: removing existing file at", source.path)
            }
            session.avExportSession = Self.convertToType(source: source, destination: destination, type: AVFileType(fileType.fileExtension), completion: { [weak self] outputURL in
            ...
            let workItem = DispatchWorkItem {
                ...
                self.convertToCAF(inputURL: source, outputURL: destination) { outputURL, error in
                    if let url = outputURL {
                        print("Conversion Complete!")
                        self.finish(session: session, error: nil)
                    } else {
            ...
            let workItem = DispatchWorkItem {
                ...
                self.convertAACtoWAV(inputURL: source, outputURL: destination) { outputURL, error in
                    if let url = outputURL {
                        print("Conversion Complete!")
                        self.finish(session: session, error: nil)
                    } else {
...
//                }
    ...
    func convertAACtoWAV(inputURL: URL, outputURL: URL) {
        ...
        print("6 About to open \(inputURL) which has a status of \(FileManager.default.fileExists(atPath: inputURL.path)) which looks like this: \(inputURL as CFURL) as a CFURL")
    ...
    func convertAACtoWAV(inputURL: URL, outputURL: URL, completion: @escaping ((URL?, Error?) -> Void)) {
        var error: OSStatus = noErr
    ...
    func convertToCAF(inputURL: URL, outputURL: URL, completion: @escaping ((URL?, Error?) -> Void)) {
        var error: OSStatus = noErr
    ...
    func convertAudio(inputURL: URL, outputURL: URL, outputFormat: AudioFileTypeID, completion: @escaping ((URL?, Error?) -> Void)) {
        var error: OSStatus = noErr
