...
public class UAConverter {
    ...
    @discardableResult
    public func convert(source: URL, destination: URL, fileType: UAFileType, completion: @escaping (Error?) -> Void) -> UAConvertSession {
        let session = UAConvertSession().completion(completion)
        ...
        if fileType == .wav || fileType == .wma || fileType == .caf || fileType == .aac || fileType == .flac || fileType == .alac || fileType == .aifc || fileType == .aiff {
            session.avExportSession = Self.convertToM4a(file: source) { [self] convertedFile in
                guard let finalFile = convertedFile else {
                    debugPrint(">>> convertedFile error")
                    DispatchQueue.main.async {
                        if let block = session.state.completionBlock {
                            block(ConvertError.cannotConvert)
                            session.state.completionBlock = nil
                        } else {
                            debugPrint("⚠️ Completion block is nil for session \(session.id)")
                        }
                    }
                    return
                }
                ...
                let workItem = DispatchWorkItem {
                    if self.checkAudioFileAccessibility(fileURL: finalFile) {
                        debugPrint("Warning: removing existing file at \(finalFile)")
                    }
                    try? FileManager.default.removeItem(at: destination)
                    self.convertAudio(inputURL: finalFile, outputURL: destination, audioType: fileType.audioFileTypeID, audioFormat: fileType.audioFormatID) { outputURL, error in
                        if let _ = outputURL {
                            DispatchQueue.main.async {
                                self.finish(session: session, error: nil)
                            }
            ...
            session.avExportSession = Self.convertToM4a(file: source) { [weak self] convertedFile in
                ...
                converter.inputFile = finalFile.path
